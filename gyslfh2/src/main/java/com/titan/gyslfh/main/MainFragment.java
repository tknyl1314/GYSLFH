...
public class MainFragment extends Fragment implements IMain, CalloutInterface {
    ...
    private  void  setTouchListener(){
        mMainFragBinding.mapview.setOnTouchListener(new DefaultMapViewOnTouchListener(getActivity(),mMainFragBinding.mapview){
            ...
            public boolean onSingleTapConfirmed(MotionEvent e) {
                ...
                /*query.setGeometry(envelope);
                query.setMaxFeatures(1);
                query.setSpatialRelationship(QueryParameters.SpatialRelationship.ENVELOPE_INTERSECTS);*/
                /*query.setMaxFeatures(1);
                query.setSpatialRelationship(QueryParameters.SpatialRelationship.ENVELOPE_INTERSECTS);
                query.setOutSpatialReference(mMapView.getSpatialReference());*/

                for (ServiceFeatureTable featureTable:featuretables){
    ...
    public View createCallView( Map<String, Object> attr){
        ...
        for(String key:keys) {
            ...
            //String alias=result.getFields()..get(i).getAlias();
            calloutcontent.append(key + " | " + value + "\n");
            i++;
        }
       /* mCalloutBinding= DataBindingUtil.setContentView(getActivity(),R.layout.callout);
        //CalloutBinding.inflate(getLayoutInflater())
        CalloutViewModel calloutViewModel=new CalloutViewModel(getActivity(),this);
        calloutViewModel.attr.set(calloutcontent.toString());
        if(keys.contains("MONITORIP")){
            calloutViewModel.ismonitor.set(true);

        }
        mCalloutBinding.setViewmodel(calloutViewModel);*/
        //如果是监控点显示视频监控
        if(keys.contains("MONITORIP")){
    ...
    @Override
    public void onAlarm() {
        startActivity(new Intent(getActivity(),UpAlarmActivity.class));
    }
